...
namespace ImGui
{
    ...
    IMGUI_API void          DestroyContext(ImGuiContext* ctx = NULL, char const *ini_file_path = NULL); // NULL = destroy current context
    ...
    IMGUI_API void          NewFrame(char const *ini_file_path);        // start a new Dear ImGui frame, you can submit any command from this point until Render()/EndFrame().
    ...
    IMGUI_API const ImGuiPayload*   AcceptDragDropPayload(const char* type, ImGuiDragDropFlags flags = 0,
                                                          ImRect *target_rect_out = nullptr,
                                                          bool visual_rect_sides_TBRL[4] = nullptr,
                                                          ImVec2 offset_visual_min_max_rect = { 0, 0 });            // accept contents of a given type. If ImGuiDragDropFlags_AcceptBeforeDelivery is set you can peek into the payload before the mouse button is released.
    ...
    IMGUI_API bool          IsItemHovered(ImGuiHoveredFlags flags = 0, float hover_delay = 0);  // is the last item hovered? (and usable, aka not blocked by a popup, etc.). See ImGuiHoveredFlags for more options.
    ...
    IMGUI_API ImRect        GetItemRect();                                                      // get bounding rectangle of the last item (screen space)
    ...
    IMGUI_API bool          IsMouseHoveringRect(const ImRect& r, bool clip = true);                          // is mouse hovering given bounding rect (in screen space). clipped by current clipping settings, but disregarding of other consideration of focus/window ordering/popup-block.
