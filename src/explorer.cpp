...
static ImVec4 const dir_color = get_color(basic_dirent::kind::directory);
static ImVec4 const symlink_color = get_color(basic_dirent::kind::symlink);
static ImVec4 const file_color = get_color(basic_dirent::kind::file);
...
bool render_rename_entry_popup_modal(
    ...
    bool &open) noexcept
{
    ...
    return rename_triggered;
}

bool render_bulk_rename_popup_modal(
    explorer_window &expl,
    ...
    ImGuiTableSortSpecs *sort_specs) noexcept
{
    ...
    if (rename_button_pressed && pattern_compile_res.success && collisions.empty() && rename_state.load() == bulk_rename_state::nil) {
        ...
        // TODO: check num_transform_errors
        if (collisions.empty()) {
    ...
    return retval_rename_completed;
}

void render_error_popup_modal(std::string &action, std::string &failure, bool &open) noexcept
{
    namespace imgui = ImGui;
    ...
    failure.front() = (char)toupper(failure.front());
    if (failure.back() != '.') {
        failure.push_back('.');
    }
    ...
    if (action.empty()) {
        imgui::TextColored(red, "%s", failure.c_str());
    }
    else {
        action.front() = (char)toupper(action.front());
        if (action.back() != '.') {
            action.push_back('.');
        }

        imgui::TextUnformatted("Failed:");
        imgui::TextColored(orange, "%s", action.c_str());
        imgui::Spacing();
        imgui::Spacing();
        ...
        if (!filter_is_blank) {
            ...
            switch (expl.filter_mode) {
                ...
                case explorer_window::filter_mode::contains: {
                    ...
                    for (auto &dirent : expl.cwd_entries) {
                        dirent.is_filtered_out = !matcher(dirent.basic.path.data(), expl.filter.data());
                    }
                    break;
                }
                ...
                case explorer_window::filter_mode::regex: {
                    ...
                    for (auto &dirent : expl.cwd_entries) {
                        dirent.is_filtered_out = !std::regex_match(
...
// TODO: this needs a refactor. we should not be doing a bunch of work here nor calling `update_cwd_entries`.
    ...
    if (expl.wd_history.empty()) {
        ...
        if (imgui::BeginTable("history_table", 3)) {
            ...
            for (auto iter = expl.wd_history.rbegin(); iter != expl.wd_history.rend(); ++iter, --i, ++i_inverse) {
                ...
                char buffer[2048] = {};
                snprintf(buffer, lengthof(buffer), "%s##%zu", hist_path.data(), i);
...
void swan_render_window_explorer(explorer_window &expl) noexcept
{
    ...
    bool any_popups_open = open_rename_popup || open_bulk_rename_popup || open_error_popup;
    ...
    if (any_window_focused && !any_popups_open && imgui::IsKeyPressed(ImGuiKey_Enter)) {
        if (explorer_window::NO_SELECTION == expl.cwd_prev_selected_dirent_idx) {
            ...
            error_popup_action = "[Enter] was pressed (function: open)";
            error_popup_failure = "nothing is selected";
        } else {
    ...
    if (any_window_focused && !any_popups_open && imgui::IsKeyPressed(ImGuiKey_F2)) {
        if (explorer_window::NO_SELECTION == expl.cwd_prev_selected_dirent_idx) {
            ...
            open_error_popup = true;
            error_popup_action = "[F2] was pressed (function: rename)";
            error_popup_failure = "nothing is selected";
        } else {
        ...
        for (auto const &dirent : expl.cwd_entries) {
            if (dirent.is_selected && !dirent.basic.is_dotdot()) {
                ...
                if (path_append(src, dirent.basic.path.data(), dir_sep_utf8, true)) {
                    ...
                    open_error_popup = true;
                    error_popup_action = std::string("add ") + dirent.basic.kind_cstr() + " [" + dirent.basic.path.data() + "] to Cut clipboard";
                    error_popup_failure = "max path length exceeded when trying to append name to cwd";
                    s_paste_payload.items.clear();
                    break;
                }
            }
        ...
        for (auto const &dirent : expl.cwd_entries) {
            if (dirent.is_selected && !dirent.basic.is_dotdot()) {
                ...
                if (path_append(src, dirent.basic.path.data(), dir_sep_utf8, true)) {
                    ...
                    open_error_popup = true;
                    error_popup_action = std::string("add ") + dirent.basic.kind_cstr() + " [" + dirent.basic.path.data() + "] to Copy clipboard";
                    error_popup_failure = "max path length exceeded when trying to append name to cwd";
                    s_paste_payload.items.clear();
                    break;
                }
    ...
    // filter type start
    {
        ...
        imgui::Combo("##filter_mode", (s32 *)(&expl.filter_mode), filter_modes, lengthof(filter_modes));
        imgui::PopItemWidth();
    }
    // filter type end
    ...
    static std::vector<explorer_window::dirent *> cwd_entries_selected = {};
    cwd_entries_selected.clear();
    u64 num_selected_directories = 0;
    u64 num_selected_symlinks = 0;
    u64 num_selected_files = 0;

    static std::vector<explorer_window::dirent *> cwd_entries_passing_filter = {};
    cwd_entries_passing_filter.clear();
    ...
    for (auto &dirent : expl.cwd_entries) {
        ...
        if (!dirent.is_filtered_out) {
            cwd_entries_passing_filter.push_back(&dirent);
            if (dirent.is_selected) {
                cwd_entries_selected.push_back(&dirent);
            }
        }
    }
    ...
    if (path_is_empty(expl.cwd)) {
        ...
        if (imgui::BeginChild("cwd_entries_child", ImVec2(0, imgui::GetContentRegionAvail().y))) {
            if (num_filtered_dirents == expl.cwd_entries.size()) {
                ...
                if (cwd_entries_sort_specs != nullptr && (expl.needs_sort || cwd_entries_sort_specs->SpecsDirty)) {
                    {
                        ...
                        sort_cwd_entries(expl, cwd_entries_selected, cwd_entries_sort_specs);
                    }
                ...
                ImGuiListClipper clipper;
                assert(cwd_entries_passing_filter.size() <= (u64)INT32_MAX);
                clipper.Begin((s32)cwd_entries_passing_filter.size());

                while (clipper.Step())
                for (u64 i = clipper.DisplayStart; i < clipper.DisplayEnd; ++i) {
                    auto &dirent = *cwd_entries_passing_filter[i];
                            ...
                            if (seconds_between_clicks <= double_click_window_sec && path_equals_exactly(current_click_path, last_click_path)) {
                                if (dirent.basic.is_directory()) {
                                    ...
                                    if (res.success) {
                                        ...
                                        open_error_popup = true;
                                        error_popup_action = std::string("open symlink [") + dirent.basic.path.data() + "]";
                                        error_popup_failure = res.error_or_utf8_path;
                                    }
                                }
                                else {
                                    ...
                                    if (res.success) {
                                        ...
                                        open_error_popup = true;
                                        error_popup_action = std::string("open file [") + dirent.basic.path.data() + "]";
                                        error_popup_failure = res.error_or_utf8_path;
                                    }
                                }
                            }
                ...
                if (imgui::BeginPopup("Context")) {
                    ...
                    if (imgui::Selectable("Copy full path")) {
                        ...
                        if (!path_append(full_path, right_clicked_ent->basic.path.data(), dir_sep_utf8, true)) {
                            open_error_popup = true;
                            error_popup_action = std::string("copy full path of [") + right_clicked_ent->basic.path.data() + "]";
                            error_popup_failure = "max path length exceeded when appending name to cwd";
                        } else {
                    ...
                    if (imgui::Selectable("Reveal in File Explorer")) {
                        ...
                        if (!res.success) {
                            open_error_popup = true;
                            error_popup_action = std::string("reveal [") + right_clicked_ent->basic.path.data() + "] in File Explorer";
                            error_popup_failure = res.error_or_utf8_path;
                        }
                    }
    ...
    if (imgui::BeginPopupModal("Rename entry", &open_rename_popup, ImGuiWindowFlags_AlwaysAutoResize)) {
        bool rename_triggered = render_rename_entry_popup_modal(*dirent_to_be_renamed, expl, dir_sep_utf16, open_rename_popup);
        if (rename_triggered) {
            update_cwd_entries(full_refresh, &expl, expl.cwd.data());
            // ^^ we just invalidated all pointers, so set them all to nullptr
            // (will help to catch any malformed usage of them by crashing the program)
            memset(cwd_entries_passing_filter.data(), (s32)nullptr, cwd_entries_passing_filter.size() * sizeof(void*));
            memset(cwd_entries_selected.data(),       (s32)nullptr, cwd_entries_selected.size()       * sizeof(void*));
        }
    }
    ...
    if (imgui::BeginPopupModal("Bulk rename", nullptr)) {
        bool rename_finished = render_bulk_rename_popup_modal(expl, cwd_entries_selected, dir_sep_utf16, open_bulk_rename_popup, cwd_entries_sort_specs);
        if (rename_finished) {
            update_cwd_entries(full_refresh, &expl, expl.cwd.data());
            // ^^ we just invalidated all pointers, so set them all to nullptr
            // (will help to catch any malformed usage of them by crashing the program)
    ...
    if (imgui::BeginPopupModal("Error", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
        render_error_popup_modal(error_popup_action, error_popup_failure, open_error_popup);
    }
